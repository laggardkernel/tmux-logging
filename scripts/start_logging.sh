...
pipe_pane_ansifilter() {
	tmux pipe-pane "exec cat - | tee -a ${FILE%.log}-raw.log | ansifilter >> $FILE"
}
...
|]0;[^]+|[[:space:]]+$)"
	tmux pipe-pane "exec cat - | tee -a ${FILE%.log}-raw.log | sed -E \"s/$ansi_codes_osx//g\" >> $FILE"
}

pipe_pane_sed() {
	local ansi_codes="(\x1B\[([0-9]{1,2}(;[0-9]{1,2})?)?[m|K]|
)"
	tmux pipe-pane "exec cat - | tee -a ${FILE%.log}-raw.log | sed -r 's/$ansi_codes//g' >> $FILE"
}
